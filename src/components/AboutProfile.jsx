...
const AboutProfile = () => {
  ...
  const [skills, setSkills] = useState([
    { skillName: "English Language", level: "70" },
    { skillName: "Uzbek Language", level: "90" },
    { skillName: "Russian Language", level: "80" },
  ...
  return (
    <div>
      <div className='flex justify-between items-center w-full border-2 border-sky-400 p-10 mt-5 rounded-3xl shadow-inner shadow-sky-400 flex-col gap-5'>
        ...
        <div className='flex gap-5 items-center flex-wrap justify-between w-full mt-5'>
          {skills.map((item, id) => (
            <div className="flex flex-col gap-5 flex-1 w-1/3" key={id}>
              <h3>{item.skillName}</h3>
              <progress className="progress progress-info w-full" value={item.level} max="100"></progress>
            </div>
          ))}
        </div>
      </div>
