...
const About = () => {
  ...
  const [SoftSkill, setSoftSkill] = useState(["Bilingaul or multilingual", "Database management", "Adobe software suitle", "Data mixing", "Ux design", "Data"])
  const [HardSkills, setHardSkills] = useState([
    "Html", "CSS", "Responsive Web Site", "Adaptive Web Site", "Tailwind", "Ant-design", "MUI", "Chakra", "Bootstrap", "Reactstrap", "SCSS", "Vite", "Javascript", "Api", "React", "React Router", "Axios", "framer Motion", "Aos", "Styled-components", "Node.js"])
        ...
        <div className='w-4/5'>
          ...
          <div className='flex w-full border-2 border-sky-400 p-10 rounded-3xl shadow-inner shadow-sky-400 justify-between flex-col gap-5'>
            ...
            <p className='text-info text-opacity-50 tracking-wide '>
              Hello My name is <span className='border-2  text-yellow-500 rounded-lg px-2 font-bold'>IBROKHIMGEY</span>, my surname is <span className='border-2 text-yellow-500 rounded-lg px-2 font-bold'>JURAEV</span>. I study in Mars IT School.
          ...
          <div className="flex gap-5">
            <div className='flex justify-between items-left w-full border-2 border-sky-400 p-10 mt-5 rounded-3xl shadow-inner shadow-sky-400 flex-col gap-5'>
              <h1 className='font-bold text-blue-500'>Hard Skills</h1>
              <div className='flex flex-wrap gap-1'>
                {HardSkills.map(item => (
                  item.toLowerCase().includes('react') ? <div key={HardSkills.indexOf(item)} className='badge badge-warning animate-pulse'>{item}</div> : <div key={HardSkills.indexOf(item)} className='badge badge-primary'>{item}</div>
                ))}
              </div>
            </div>

            <div className='flex justify-between items-left w-full border-2 border-sky-400 p-10 mt-5 rounded-3xl shadow-inner shadow-sky-400 flex-col gap-5'>
              <h1 className='font-bold text-blue-500'>Soft Skills</h1>
              <div>
                {SoftSkill.map(item => (
